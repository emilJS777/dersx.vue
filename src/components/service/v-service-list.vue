<template>
  <div class="service bg-fff padding-1 h-max-content">
      <div class="service_img_block p-relative o-hidden c-pointer" v-if="service.image">
        <img :src="'data:image/'+service.image.filename+';charset=utf-8;base64, ' + service.image.b64" class="p-absolute absolute-center" alt="">
      </div>

      <h4 class="m-top-05 c-content-hover c-pointer">{{service.title}}</h4>
      ...
      <div class="d-flex j-content-space-between">
        ...
        <span class="f-size-small">{{service.rubric.title}}</span>
      </div>
  </div>
</template>
