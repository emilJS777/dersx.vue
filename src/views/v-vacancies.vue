<template>
  <div class="vacancies d-grid g-gap-3">
    ...
    <div class="vacancies_list d-grid g-gap-1 h-max-content">
      ...
      <v-vacacies-list @more="setModalName('vacancyModal')"
                       class="bg-fff padding-1"
                       v-for="vacancy in vacancies" :key="vacancy.id"
                       :vacancy="vacancy"/>

      <h3 class="t-center c-555" v-if="!vacancies.length">ничего не найдено</h3>
      <!--    PAGINATION-->
  ...
  <v-vacancy-modal v-if="modalName === 'vacancyModal'" @close="setModalName(false)"/>
  <v-vacancy-edit-modal v-if="modalName === 'vacancyEditModal'" @close="setModalName(false)"/>
</template>
...
import VVacancyEditModal from "@/components/vacancy/v-vacancy-edit-modal";
